...
def unpack(packet, skip_crc8=False):
    ...
    if len(packet) < packet_correct_size:
        return {'unused_packet': unused_packet}
    packet, unused_packet = (packet[0:packet_correct_size], packet[packet_correct_size:])
    ...
    # parse data
    protocol_data['name'] = next(x for x,y in protocol_info_table.items() if y.get('id','')==protocol_data['id'])
    protocol_data['data'] = packet_to_dict(protocol_data['name'], packet[7-skip_crc8:-2])
    ...
    def update(self, packet, verbose=False):
        ...
        if verbose:
            if 'error' in unpack_result:
                ...
                print('LOG Protocol: ', unpack_result['result']['name'], unpack_result['result']['data'])
...
if __name__ == '__main__':
    ...
    print('unpack result: ', unpack(bytes([1,2,3])+test_packet+bytes([4,5,6]))) # test with some useless bytes